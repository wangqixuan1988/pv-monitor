...
//#endif
		...
		if (recode != SQLITE_OK)
		{
			if( strcmp(err_msg,"table collect_env already exists"))
			{
				...
				printf("table collect_env already exist,so open it ok!\n ");
		}
//		else
//		{
#if DEBUG_SQLITE
...
//		}
		...
		if (recode != SQLITE_OK)
		{
			if( strcmp(err_msg,"table goods already exists"))
			{
				...
				printf("table goods already exits,so open it ok");
		}
...
int addGoods (struct env_info_clien_addr env_info_clien_addr_t, int storageNum_t, int goodsKinds_t)
{
	...
	else
	{
		...
		env_info_clien_addr_s.storage_no[storageNum_t].goods_info[goodsKinds_t].goods_type = (int)callback_val[1];
		env_info_clien_addr_s.storage_no[storageNum_t].goods_info[goodsKinds_t].goods_count = (int)callback_val[2];
//		printf ("storage num is %d\ngoodstype is %d\ngoodscount is %d\n",storageNum_t,env_info_clien_addr_s.storage_no[storageNum_t].goods_info[goodsKinds_t].goods_type, env_info_clien_addr_s.storage_no[storageNum_t].goods_info[goodsKinds_t].goods_count);
	}
